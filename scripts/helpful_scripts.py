...
def get_account(index=None, id=None, account=None):
    if index:
...
def issue_tokens():
    """You can call this function once you have deployed your FarmToTheMoon contract to a live network
    ...
    Run this function with this command: `brownie run scripts/issue_tokens.py --network kovan`
        This function will:
            - Print your account address and deployed FarmToTheMoon contract address to confirm that you're using the right ones
            - Call issueTokens on your deployed FarmToTheMoon contract to issue the DAPP token reward to your users
    """
    account = get_account()
    print(f"Issue Tokens called by: {account}")
    token_farm = FarmToTheMoon[-1]
    print(f"FarmToTheMoon contract called to issue tokens: {token_farm}")
    tx = token_farm.sendRewards({"from": account})
    tx.wait(1)
